...
export const saveEvaluation = (evaluationData) => {
  try {
    ...
    const newEvaluation = {
      ...
      status: 'pending', // pending, under_review, accepted, rejected, sanggah
...
export const updateEvaluationStatus = (id, status, data = {}) => {
  try {
    ...
    if (status === 'accepted') {
      ...
      // Calculate sanggah deadline (5 working days from rejection)
      const deadline = addWorkingDays(new Date(), 5);
      evaluation.sanggahDeadline = deadline.toISOString();
      evaluation.status = 'sanggah'; // Auto set to sanggah status for 5 days
...
// Submit sanggahan (User action)
export const submitSanggah = (id, sanggahData) => {
  try {
    if (typeof window === 'undefined') return false;
    ...
    // Check if still within sanggah period
    if (evaluation.sanggahDeadline) {
      const deadline = new Date(evaluation.sanggahDeadline);
      const now = new Date();

      if (now > deadline) {
        alert('Masa sanggah telah berakhir!');
        return false;
      }
    }

    evaluation.sanggahSubmitted = true;
    evaluation.sanggahNotes = sanggahData.notes;
    evaluation.sanggahDocuments = sanggahData.documents || [];
    evaluation.status = 'under_review'; // Back to review after sanggah
    evaluation.sanggahSubmittedAt = new Date().toISOString();
    ...
    console.error('Error submitting sanggah:', error);
    return false;
  }
};

// Check if sanggah period is expired
export const checkSanggahExpired = (evaluation) => {
  if (!evaluation.sanggahDeadline) return false;
